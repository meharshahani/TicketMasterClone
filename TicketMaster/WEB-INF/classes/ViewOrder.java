...
protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
{
		...
		// try
		// {
		// 	FileInputStream fileInputStream = new FileInputStream(new File(TOMCAT_HOME+"\\webapps\\Assignment1_old\\PaymentDetails.txt"));
		...
		if(request.getParameter("Order")!=null && request.getParameter("Order").equals("ViewOrder"))
		{
			...
			// {	
				...
				{	
					...
					//pw.print("<td>Delivery Date:</td></tr>");
					for (OrderPayment oi : orderPayments.get(orderId)) 
					{
						...
						pw.print("<td><input type='hidden' name='deldate' value='"+oi.getOrderId()+"'></td>");
		...
		if(request.getParameter("Order")!=null && request.getParameter("Order").equals("CancelOrder"))
		{
			...
			if(request.getParameter("orderName") != null)
			{
				...
				// try
				// {	
				// 	FileOutputStream fileOutputStream = new FileOutputStream(new File(TOMCAT_HOME+"\\webapps\\Assignment1_old\\PaymentDetails.txt"));
