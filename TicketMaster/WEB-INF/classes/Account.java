		...
		try
         {  
           response.setContentType("text/html");
			...
			//String num_tickets = session.getAttribute("num_tickets").toString();	
			...
			try
				{
					// FileInputStream fileInputStream = new FileInputStream(new File(TOMCAT_HOME+"\\webapps\\TicketMaster\\PaymentDetails.txt"));
			...
			if(size>0)
				{	
					...
					pw.print("<td>Movie Booked:</td>");
					pw.print("<td>Ticket Price:</td>");
					//pw.print("<td>Number of tickets:</td></tr>");
					...
					for(Map.Entry<Integer, ArrayList<OrderPayment>> entry : orderPayments.entrySet())
					{
						...
						if(oi.getUserName().equals(user.getName())) 
						{
							...
							pw.print("<td>"+oi.getOrderId()+".</td><td>"+oi.getUserName()+"</td><td>"+oi.getOrderName()+"</td><td> "+oi.getOrderPrice()+"</td>");
							pw.print("<td><input type='hidden' name='orderId' value='"+oi.getOrderId()+"'></td>");
							//pw.print("<td><input type='hidden' name='num_tickets' value=num_tickets></td>");
							//System.out.println(num_tickets);
